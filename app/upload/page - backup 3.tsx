...
export default function UploadPage() {
  ...
  const [aiPlan, setAiPlan] = useState<"fast" | "pro" | "cinema">("fast");
...
const [templateBGM, setTemplateBGM] = useState<string>("");
...
const previewAudioRef = useRef<HTMLAudioElement | null>(null);
  ...
  const handleAIGenerate = async () => {
    if (!aiPrompt.trim() || aiPrompt.trim().length < 3) { alert("Prompt must be at least 3 characters"); return; }
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/ai/generate-video`, {
        ...
        body: JSON.stringify({ prompt: aiPrompt, plan: aiPlan, duration: aiDuration, user_id: user.id }),
      });
      const data = await res.json();
      if (data.success) {
        setAiVideoUrl(data.video_url);
        // 下载一份作为 selectedFile 用于预览
...
const synthesizeMemoryTemplate = async () => {
  ...
  if (templateBGM) {
   try {
      const res = await fetch(templateBGM);
      ...
      bgmSource.connect(audioDest); 
      // 不连接 destination，合成时不出声
      bgmSource.start(0);
    } catch (e) {
      console.warn("BGM 加载失败，静音合成", e);
    }
  }
...
const handleGenerateCaptions = async () => {
  ...
  try {
    ...
    if (aiVideoUrl) {
      fd.append("video_url", aiVideoUrl);
      fd.append("source", "ai");
    } else {
          ...
          {sourceMode === "ai" && (
            <div className="bg-gray-900 rounded-xl p-4 mb-4 space-y-3">
              ...
              <div className="flex items-center gap-3 text-xs text-gray-400">
                <span>Quality:</span>
                {(["fast","pro","cinema"] as const).map(p => (
                  <button key={p} onClick={() => setAiPlan(p)} className={`px-2 py-1 rounded ${aiPlan===p?"bg-yellow-400 text-black":"bg-gray-800"}`}>
                    {p==="fast"?"⚡Fast":p==="pro"?"🎬Pro":"👑Cinema"}
                  </button>
                ))}
                <span className="ml-auto">{aiPlan==="fast"?5:aiPlan==="pro"?70:200} 💎</span>
              </div>
              <div className="flex items-center gap-2 text-xs text-gray-400">
                <span>Duration:</span>
                <input type="range" min={5} max={30} value={aiDuration} onChange={e=>setAiDuration(+e.target.value)} className="flex-1 accent-yellow-400" />
                <span>{aiDuration}s</span>
              </div>
              <button
                onClick={handleAIGenerate}
                disabled={aiGenerating || !aiPrompt.trim()}
                className={`w-full py-2 rounded-lg text-sm font-bold ${aiGenerating||!aiPrompt.trim()?"bg-gray-700 text-gray-500":"bg-yellow-400 text-black"}`}
              >
...
<div className="bg-gray-800 rounded-lg p-3 mb-3">
  ...
  <button
  onClick={() => {
    if (!templateBGM) return;
    const current = previewAudioRef.current;
    // 如果已有音频，且是同一首歌 → toggle 播放/暂停
    if (current && current.src.includes(templateBGM)) {
      if (current.paused) {
        current.play().catch(() => {});
      } else {
        current.pause();
