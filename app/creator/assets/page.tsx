  ...
  const loadAll = async () => {
    ...
    try {
      ...
      const [
		...
		fetch(
		`${API_BASE}/api/marketplace/my-listings/${user.id}`
        ),
...
const filteredMarketplaceHistory =
  marketplaceFilter === "all"
    ? marketplaceHistory
    : marketplaceHistory.filter(
        (item) => item.status === marketplaceFilter
      );
...
<div className="flex gap-2 mb-3 overflow-x-auto">
  ...
  <button
    onClick={() =>
      setMarketplaceFilter("cancelled")
    }
    className={`
      px-3
      py-2
...
<div className="space-y-3">
  ...
  {filteredMarketplaceHistory.map((item) => (
