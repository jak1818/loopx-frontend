...
<div
  ...
  className={`h-[100dvh] w-full overflow-y-auto snap-mandatory overscroll-none bg-black ${
...
/>
  ...
  router.push(`/profile/${video.creator_id}`);
  ...
  handleFollow(video.creator_id, video.id);
  ...
  handleLike(video);
                  ...
                  >
                    <span className="text-2xl">{likedMap[video.id] ? "❤️" : "🤍"}</span>
                    <span className="text-sm">{Number(likesMap[video.id] || 0)}</span>
                  </div>
  ...
  handleBookmark(video.id);
                  ...
                  >
                    <span className="text-xl">
                      <Bookmark
                        className={`w-6 h-6 ${bookmarkMap[video.id] ? "fill-white" : "stroke-white"}`}
                      />
                    </span>
                  </div>
