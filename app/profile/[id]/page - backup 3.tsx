...
export default function ProfilePage({ params }: any) {
  ...
  useEffect(() => {
    fetch(`http://localhost:5000/api/user/profile-full/${id}?viewer_id=${currentUserId}`)
      .then((res) => res.json())
      .then((res) => {
        if (res.success) {
          setData(res);
  ...
  const handleFollowToggle = async () => {
    ...
    const endpoint = "http://localhost:5000/api/follow";
    try {
  ...
  const saveName = async () => {
    try {
      ...
      const json = await res.json();
      console.log("saveName response:", json);
      if (json.success) {
        setData((prev: any) => ({
          ...prev,
  ...
  const saveBio = async () => {
    try {
      ...
      const json = await res.json();
      console.log("saveBio response:", json);
      if (json.success) {
        setData((prev: any) => ({
          ...prev,
  ...
  <>
    <div className="flex gap-3 mt-4">
      <button
        ...
        className="bg-gray-800 px-4 py-1.5 rounded-full text-xs border border-gray-600"
      >
        💰 Balance
      </button>
      <button
    ...
    <button
      ...
      className="mt-3 w-48 bg-pink-600 hover:bg-pink-700 px-4 py-2 rounded-full text-sm font-bold transition"
    >
      👥 Invite Friends
    </button>
  </>
